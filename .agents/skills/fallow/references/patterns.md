...
- knip `ignore` → fallow `ignoreFindings` (matching dead-code findings are hidden, but matching files remain in the module graph; leading `!` exceptions are preserved; multi-source findings stay visible unless every source owner matches)
